...
const PENDING = [{"date":"2026-09-07","items":{"fr":["Chaque profil se synchronise avec le compte. Jusqu'ici seul le premier profil suivait le compte, les autres restaient sur leur appareil. Désormais chaque profil a ses propres données sur le serveur, et la liste des profils voyage avec : un profil créé sur l'iPad apparaît sur le téléphone connecté au même compte, avec son journal, ses statuts et ses doigtés ; un profil supprimé quelque part l'est partout."],"en":["Every profile syncs with the account. Until now only the first profile followed the account, the others stayed on their device. Each profile now has its own data on the server, and the list of profiles travels along: a profile created on the iPad shows up on the phone signed in to the same account, with its journal, statuses and fingerings; a profile removed anywhere is removed everywhere."]}},{"date":"2026-09-06","items":{"fr":["Un avis part avec l'image de l'écran qu'on avait sous les yeux. Un problème se décrivait mal en mots : la mesure fautive n'a pas de nom, et le mode de jeu, le bandeau affiché ou l'état de la page ne se racontent pas non plus. La fenêtre d'avis joint maintenant une image de l'écran tel qu'il était au moment où on l'a ouverte — la fenêtre elle-même en moins — depuis n'importe quelle page, la bibliothèque comprise. Elle est montrée dans le formulaire avant l'envoi, une case permet de la retirer, et si la capture échoue le message part quand même.","Le clavier se connecte tout seul une fois appairé. Sur iPad, l'appairage Bluetooth se fait dans la fenêtre du système, hors de l'app : au retour, les notes passaient déjà, mais l'en-tête proposait toujours « Connecter clavier MIDI » et il fallait le redemander pour que l'app en convienne. Elle écoute maintenant l'arrivée du clavier — l'en-tête affiche son nom et la fenêtre d'aide se ferme d'elle-même. Un clavier débranché en cours de séance est signalé aussitôt, de la même façon.","▶ Écouter a son propre bandeau. Le morceau qu'on écoute se pilotait avec un seul bouton : ▶ pour partir du début, ⏹ pour tout arrêter. Un bandeau apparaît maintenant sous les onglets, de la même forme que celui du mode strict, avec ⏸ / ▶ pour suspendre le morceau et le reprendre à la mesure où on l'a laissé, le tempo d'écoute — modifiable en cours de lecture, sans repartir du début, et retenu pour la partition — et un clic sur n'importe quelle mesure pour écouter à partir de là, en marche comme en pause.","Travaillez un passage entier en mode entraînement. La sélection ne portait que sur une mesure ; le bouton 🔁 de la barre d'entraînement permet maintenant de cliquer la première puis la dernière mesure d'un passage, qui doit sortir trois fois de suite sans faute — enchaînements et liaisons compris."],"en":["Feedback goes out with a picture of the screen you were looking at. A problem was hard to put into words: the offending bar has no name, and neither does the mode you were in, the band that was up, or the state the page was in. The feedback window now attaches a picture of the screen as it was when you opened it — the window itself excepted — from any page, the library included. It is shown in the form before you send it, a checkbox takes it back off, and if the capture fails the message still goes.","The keyboard connects on its own once paired. On an iPad the Bluetooth pairing happens in the system sheet, outside the app: on the way back the notes were already coming through, but the header still offered \"Connect MIDI keyboard\" and you had to ask again before the app agreed. It now listens for the keyboard arriving — the header shows its name and the help sheet closes itself. A keyboard unplugged mid-session is reported just as promptly.","▶ Écouter has a band of its own. The piece being listened to was driven by a single button: ▶ from the top, ⏹ to end it. A band now appears under the tabs, the same shape as strict mode's, with ⏸ / ▶ to hold the piece and pick it up at the bar it was left at, the listening tempo — changed while it plays, without going back to the top, and remembered for the score — and a click on any bar to hear the piece from there, playing or paused.","Work a whole passage in training mode. Only one bar could be selected; the 🔁 button in the training band now lets you click a passage's first bar and then its last, and the whole thing has to come out three times in a row without a mistake — the joins and slurs between bars included."]}}]
